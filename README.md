...
<body>
    <div class="container">
        <header>
            ...
            <p class="subtitle">ΣFx = 0 | ΣFy = 0 | ΣM = 0 | 互動式靜力學分析工具</p>
        </header>
        ...
        <div class="main-content">
            ...
            <div class="output-section">
                ...
                <div class="visualization">
                    <h3><i class="fas fa-project-diagram"></i> 外力視覺化圖</h3>
                    <p>顯示外力方向與作用點位置（示意圖，非等比例）</p>
                    <div id="forceDiagram">
                        <!-- 圖表將由JavaScript繪製 -->
                        <div id="diagramContent" style="position: relative; width: 100%; height: 100%;"></div>
                    </div>
                    <div style="margin-top: 15px; font-size: 0.9rem; color: #666;">
                        <p><span style="display: inline-block; width: 12px; height: 12px; background-color: #3498db; border-radius: 50%; margin-right: 5px;"></span> F₁ <span style="margin-left: 15px; margin-right: 5px;">■</span> F₂ <span style="margin-left: 15px; margin-right: 5px; color: #2ecc71;">■</span> F₃</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <footer>
            <p>靜力學平衡方程計算器 - ΣFx=0, ΣFy=0, ΣM=0 | 適用於土木工程、機械工程靜力學學習</p>
    ...
    <script>
        ...
        function drawForceDiagram() {
            ...
            // 確定圖表範圍
            const maxX = Math.max(...forces.map(f => Math.abs(f.x))) + 2;
            const maxY = Math.max(...forces.map(f => Math.abs(f.y))) + 2;
            ...
            // X軸
            diagramContent.innerHTML += `
                <div style="position: absolute; left: 50px; top: ${centerY}px; width: 200px; height: 1px; background-color: #999;"></div>
                <div style="position: absolute; left: 250px; top: ${centerY-5}px; color: #666; font-size: 12px;">X</div>
            `;
            
            // Y軸
            diagramContent.innerHTML += `
                <div style="position: absolute; left: ${centerX}px; top: 25px; width: 1px; height: 200px; background-color: #999;"></div>
                <div style="position: absolute; left: ${centerX-10}px; top: 25px; color: #666; font-size: 12px;">Y</div>
            `;
            
            // 原點 (0,0)
            diagramContent.innerHTML += `
                <div style="position: absolute; left: ${centerX-3}px; top: ${centerY-3}px; width: 6px; height: 6px; background-color: #333; border-radius: 50%;"></div>
                <div style="position: absolute; left: ${centerX+8}px; top: ${centerY-8}px; color: #333; font-size: 11px;">(0,0)</div>
            `;
            ...
            forces.forEach((force, index) => {
                ...
                const forceLabel = `F${force.id}`;
                
                // 計算圖表中的位置
                const diagramX = centerX + (force.x * scale);
                const diagramY = centerY - (force.y * scale); // Y軸反轉，因為CSS座標向下為正
                
                // 計算箭頭分量
                const components = calculateForceComponents(force.magnitude, force.angle);
                const arrowLength = Math.min(force.magnitude / 10, 80); // 箭頭長度縮放
                const arrowAngle = force.angle * Math.PI / 180;
                
                const arrowEndX = diagramX + (arrowLength * Math.cos(arrowAngle));
                const arrowEndY = diagramY - (arrowLength * Math.sin(arrowAngle)); // Y軸反轉
                
                // 繪製作用點
                diagramContent.innerHTML += `
                    <div style="position: absolute; left: ${diagramX-4}px; top: ${diagramY-4}px; width: 8px; height: 8px; background-color: ${color}; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 3px rgba(0,0,0,0.5);"></div>
                `;
                
                // 繪製外力箭頭
                // 箭頭線段
                diagramContent.innerHTML += `
                    <div style="position: absolute; left: ${diagramX}px; top: ${diagramY}px; width: ${arrowLength}px; height: 2px; background-color: ${color}; transform-origin: 0 0; transform: rotate(${-arrowAngle}rad);"></div>
                `;
                
                // 箭頭頭部（簡化版）
                diagramContent.innerHTML += `
                    <div style="position: absolute; left: ${arrowEndX-8}px; top: ${arrowEndY-8}px; width: 16px; height: 16px; background-color: ${color}; clip-path: polygon(0% 0%, 100% 50%, 0% 100%); transform: rotate(${90-force.angle}deg);"></div>
                `;
                
                // 外力標籤
                diagramContent.innerHTML += `
                    <div style="position: absolute; left: ${diagramX+10}px; top: ${diagramY-20}px; background-color: ${color}; color: white; padding: 3px 8px; border-radius: 4px; font-size: 12px; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
                        ${forceLabel}: ${force.magnitude}N
                    </div>
                `;
            });
        }
